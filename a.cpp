...
// SOUND PATHS
// ============================================
const char* SOUND_JUMP = "D:\\c++\\hfghgfh\\bird_sound.wav";
const char* SOUND_HIT = "D:\\c++\\hfghgfh\\bird_hit.wav";
...
void drawText(float x, float y, string text, float r, float g, float b, int size) {
    ...
    // 🔥 1. Neon Glow Effect (multiple soft layers)
    for (int i = 4; i >= 1; i--) {
        float alpha = 0.05f * i;
        glColor4f(r, g, b, alpha);

        glRasterPos2f(px - i, py);
        for (char c : text) glutBitmapCharacter(font, c);

        glRasterPos2f(px + i, py);
        for (char c : text) glutBitmapCharacter(font, c);

        glRasterPos2f(px, py - i);
        for (char c : text) glutBitmapCharacter(font, c);
    ...
    // 🧱 2. Strong 8-direction outline (clean retro edge)
    glColor3f(0, 0, 0);
    int dx[] = {-1, 1, 0, 0, -1, -1, 1, 1};
    int dy[] = {0, 0, -1, 1, -1, 1, -1, 1};

    for (int i = 0; i < 8; i++) {
        glRasterPos2f(px + dx[i], py + dy[i]);
        for (char c : text) glutBitmapCharacter(font, c);
    }

    // ✨ 3. Main Text (bright core)
    glColor3f(r, g, b);
    glRasterPos2f(px, py);
    for (char c : text) glutBitmapCharacter(font, c);

    // 🌟 4. Highlight shine (top-left glow)
    glColor4f(1.0f, 1.0f, 0.8f, 0.25f);
    glRasterPos2f(px - 1, py + 1);
    for (char c : text) glutBitmapCharacter(font, c);
}
...
void drawDeveloperScreen() {
    ...
    drawText(cx - 170, SCREEN_HEIGHT - 120,
             "FLAPPY HORIZON", 1.0f, 0.85f, 0.25f, 34);

    drawText(cx - 115, SCREEN_HEIGHT - 160,
             "Soar Beyond Limits", 0.6f, 0.75f, 1.0f, 18);

    drawText(cx - 125, SCREEN_HEIGHT - 200,
             "DEVELOPER INFO", 0.75f, 0.85f, 1.0f, 20);

    int boxX = cx - 230;
    int boxY = SCREEN_HEIGHT - 500;
    int boxW = 460;
    int boxH = 420;  // Increased height to accommodate mentor
    ...
    int y = boxY + boxH - 50;
    int gap = 38;  // Slightly reduced gap to fit all content

    drawText(cx - 90, y, "DEVELOPED BY", 0.4f, 1.0f, 0.6f, 16);
    y -= gap;

    drawText(cx - 150, y, "Robiul Hasan Jisan", 1.0f, 0.9f, 0.3f, 22);
    y -= 25;
    drawText(cx - 65, y, "ID: 232008812", 0.6f, 0.6f, 0.6f, 14);

    y -= gap;

    glColor3f(0.8f, 0.8f, 0.2f);
    glBegin(GL_LINES);
    glVertex2f(boxX + 20, y);
    glVertex2f(boxX + boxW - 20, y);
    glEnd();

    y -= gap;

    drawText(cx - 130, y, "Mahbubul Alam", 1.0f, 0.9f, 0.3f, 22);
    y -= 25;
    drawText(cx - 65, y, "ID: 223011012", 0.6f, 0.6f, 0.6f, 14);

    y -= gap;

    drawText(cx - 130, y, "Inteser Hossain", 1.0f, 0.9f, 0.3f, 22);
    y -= 25;
    drawText(cx - 65, y, "ID: 232008012", 0.6f, 0.6f, 0.6f, 14);

    y -= gap;

    // Mentor section with special styling
    glColor3f(0.8f, 0.8f, 0.2f);
    glBegin(GL_LINES);
    glVertex2f(boxX + 20, y);
    glVertex2f(boxX + boxW - 20, y);
    ...
    drawText(cx - 70, y, "MENTOR", 1.0f, 0.9f, 0.4f, 18);
    y -= 28;
    drawText(cx - 85, y, "TJ Mam", 1.0f, 0.7f, 0.2f, 24);
    y -= 22;
    drawText(cx - 125, y, "Special Thanks & Guidance", 0.7f, 0.8f, 0.5f, 16);
    ...
    drawText(cx - 90, by + 20,
             "START JOURNEY", 1, 1, 1, 18);
}
void display() {
